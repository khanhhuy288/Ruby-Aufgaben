...
MNQRR XGTNCGUUV FQTVOWPF -- CNU PCEJHQNIGT YKTF VWEJGN IGJCPFGNV.'
  ...
  def test_encode()
    assert_equal(@encoded_A,@ccc.encode('A'.ord, @distance_c))
    assert_equal(@encoded_Z,@ccc.encode('Z'.ord, @distance_c))
    assert_equal(@encoded_X,@ccc.encode('X'.ord, @distance_c))
    assert_equal(@encoded_G,@ccc.encode('G'.ord, @distance_c))
      puts 'a'.ord
    assert_equal(@encoded_a,@ccc.encode('a'.ord, @distance_c))
    assert_equal(@encoded_z,@ccc.encode('z'.ord, @distance_c))
    assert_equal(@encoded_x,@ccc.encode('x'.ord, @distance_c))
    assert_equal(@encoded_g,@ccc.encode('g'.ord, @distance_c))
    assert_equal(@encoded_space,@ccc.encode(' '.ord, @distance_c))
  end
