...
class KasseTest < Test::Unit::TestCase
  ...
  def test_positionen_sort()
    assert_equal(@positionen_sortiert,@positionen.sort())
  end
  ...
  def test_rechnung_min_max()
    assert_equal(@rechnungen[2],@rechnungen.max())
    assert_equal(@rechnungen[5],@rechnungen.min())
  end
  ...
  def test_kasse_sonstiges()
    assert_equal(@kasse_sortiert,@kasse.sort(),"Falsche oder keine Ordnung")
    assert_equal( [113, 148, 88, 99, 104, 63, 103, 126, 102, 91],@kasse.map(){|rechnung| rechnung.rechnungs_betrag()})
  end

  def test_kasse_min_max()
    assert_equal(@kasse.to_a()[5], @kasse.min())
    assert_equal(@kasse.to_a()[1], @kasse.max())
  end
